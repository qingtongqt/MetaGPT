...
class MakeConsensus(Action):
    ...
    PAIR_WISE_TEMPLATE: str = """
        Given a query {query}, which of the following code is more relevant to the query?
        {role1}: {content1}
        {role2}: {content2}
        ...
        if use_algorithm:
            try:
                ...
                for i in range(1, len(sorted_role)):
                    ...
                    if role1.profile == result or role1.profile in result:
                        selected_role = role1
                    elif role2.profile == result or role2.profile in result:
                        selected_role = role2
                    else:
